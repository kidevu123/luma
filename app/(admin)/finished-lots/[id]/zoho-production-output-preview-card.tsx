...
export function ZohoProductionOutputPreviewCard({
  ...
  const [result, setResult] = React.useState<ProductionOutputPreviewActionResult | null>(null);
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const response = await previewZohoProductionOutputAction({
        ...
        purchaseorderLineItemId: String(formData.get("purchaseorder_line_item_id") ?? ""),
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="rounded-md border border-info-300/60 bg-info-50 px-3 py-2 text-xs text-info-800">
          ...
          warehouse, and item mapping before live production-output writes exist.
        </div>
        ...
        <form className="grid gap-3 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          <Field label="Zoho purchaseorder_line_item_id" htmlFor="purchaseorder_line_item_id">
            <input
...
function Field({
  ...
  return (
    <label className="space-y-1 text-xs font-medium text-text-muted" htmlFor={htmlFor}>
      <span>{label}</span>
      {children}
    </label>
  );
...
function PreviewResult({ result }: { result: ProductionOutputPreviewActionResult }) {
  ...
  const Icon = result.ok ? CheckCircle2 : result.kind === "SERVICE_ERROR" ? AlertTriangle : XCircle;

  return (
    <div className={`space-y-3 rounded-md border px-3 py-2 text-xs ${statusClass}`}>
      ...
      {"payload" in result && result.payload && (
        <JsonBlock title="Request summary sent to preview" value={result.payload} />
      )}
        ...
        <ul className="list-disc space-y-1 pl-5">
          {result.blockers.map((blocker) => (
            <li key={`${blocker.field}:${blocker.message}`}>
              <span className="font-mono">{blocker.field}</span>: {blocker.message}
            </li>
          ))}
        </ul>
      )}

      {"body" in result && result.body != null && <ResponseSummary body={result.body} />}
    </div>
  );
}
