...
- Simplify Phase E: login. Signing in from a deep link returns you there — `safeNextPath` validates every destination (site-relative only; query and hash preserved) and the guards accept an optional `{ next }`, with the five deep-linkable operational pages passing their own path (opt-in per page: the middleware approach was rejected because it forces an Edge bundle that breaks the build via instrumentation.ts). Both sign-in methods honor `?next=`. Security: the OIDC callback's open redirect is closed — `next` is validated at the SSO start AND again before `new URL()` — and the `oidc_state` CSRF cookie now sets `secure` in production like the session cookie. The login card ranks one path: a primary "Sign in" (SSO) with password behind a "Sign in with password instead" disclosure that focuses the email field, and the password placeholder no longer fakes a saved credential. Session cookie 8h → 12h so a floor tablet survives a shift plus handover.
- Known gap recorded in `lib/auth.ts`: role and disabled state still come from the signed cookie, so a demotion or disable takes up to 12h to bite. Closing it requires replacing the process-lifetime `requestCache` Map with React's `cache()` first — widening the select alone would defeat revocation and could lock out a re-enabled user.

Smoke (staging): hit `/po-closeout/<id>` signed out — after SSO you land back on that PO, not `/dashboard`; `?next=https://evil.example` and `?next=//evil.example` both land on `/dashboard`; password sign-in still works behind the disclosure with SSO configured; a session survives a 12-hour shift.
...
Smoke (staging): a PO whose only open work is a failed Zoho op shows an amber (not red) header; the mapping banner counts distinct SKUs and its Fix-mapping links land on the product page; "Open this PO's Zoho operations" shows only that PO's ops incl. NEEDS_MAPPING ones with a working Back to closeout; Queue-all-ready queues READY ops and reports skips; NOT_READY rows show no queue button.
...
Smoke (staging): closeout detail defaults to Do here with bucket counts matching visible rows; empty-bag toggle shows/hides 0-production bags; inline Issue lot on a ready bag creates + releases per eligibility; "Finalize on floor" lands on the exact bag expanded in workflows; malformed `?bag=` degrades to the unfiltered list; Release lot from hold works in one tap with empty reason; zero-bag POs render collapsed with "Open anyway" links.

## [1.36.0] — 2026-08-17

- Phase 6: the production engine overhaul closes. Route data drives queue transitions and SSE relevance with parity pins proving twin === hardcoded for every (route, stationKind) in an exhaustive matrix (card/bottle/sticker-only, bottle narrowing). HANDPACK_BLISTER is a real route operation (migration 0074): its own route_operations row on CARD_BLISTER, COMPLETE_EVENT_FOR_OPERATION entry, and station-event-mapping pins. Eight dead legacy actions retired (fireStageEventAction, setOperatorAction, verifyVendorBarcodeAction, packagingCompleteAction, lookupCardByTokenAction, finalizeBagAction, releaseSealingHandoffAction, releaseBagAction) — each proven zero-caller by grep before deletion. Damaged-packaging field restored: damagedPackaging CompletionInput key on packaging operations, mapped through buildRecordPackagingCompleteInput and reaching the Zoho payload. Queue-state double-count fixed: SEALING_QUEUE / POST_BLISTER_STAGING and PACKAGING_QUEUE / POST_SEAL_STAGING now count disjoint bag populations via read_bag_queue.queue_stage_key. Downtime resolvable: raiseDowntimeEnded engine function plus resolveDowntimeAction admin action plus [ Resolved ] button on the Act Now rail. Cleanup ledger closed: blockerForWithDetail factory for four dynamic-detail blocker codes; value-pin tests for duplicated station-kind sets; STAGE_DEFS structural pin.

Smoke (staging): route parity spot-checks (card, bottle, sticker-only bags queue and SSE-update identically pre/post switch); HANDPACK_BLISTER completes via HANDPACK_BLISTER_COMPLETE; damagedPackaging records and appears in Zoho preview payload; downtime resolve flow (DOWNTIME_STARTED on rail, click Resolved, DOWNTIME_ENDED recorded); board disambiguated counts (SEALING_QUEUE and POST_BLISTER_STAGING non-overlapping); retired action names absent from UI network tab.
...
- **Zoho-closed POs now land in Closed, not Open:** the closeout list queries the raw `zoho_status` columns introduced on `purchase_orders` (populated by the existing sync path); any PO whose Zoho status maps to the Zoho-terminal bucket (`received / closed / billed / cancelled` — confirmed against live gateway data 2026-08-04: fully-received POs carry raw status `received`, which is what Zoho presents as "closed") is rendered in the **Closed** tab with a **Zoho-closed** origin chip, distinguishing it from Luma-closed POs (all outputs pushed and committed) at a glance. The mapping is pure and unit-tested.
...
- **Hourly sync + manual refresh:** the existing `/api/cron/zoho-po-sync` route (whose sync now also stores raw `zoho_status` / `zoho_status_synced_at`) is fired hourly by the retimed `luma-zoho-po-sync` systemd timer (was daily at 03:59); a **Refresh from Zoho** button on the closeout list header reuses the existing admin server action `syncPurchaseOrdersFromZohoAction` on demand so admins do not wait up to an hour after a Zoho close.
...
- **Tests:** 3 new pure gate tests for the issue-lot mapping and open-session gating (suite now 15), plus 1 updated existing assertion.
...
- **Per-bag production breakdown (shared, read-only):** new pure module `lib/production/bag-production-summary.ts` + batch loader `lib/db/queries/bag-production-summary.ts` compute, per inventory bag, **Received / Produced / Remaining / Complete%** with honest sources and flags — from Luma's canonical data only (inventory bag counts, `read_bag_metrics` packaging output falling back to the deepest recorded stage output FINISHED > PACKAGING > SEALING, `tablets_per_unit` conversion, non-voided allocation sessions, finished lots, non-voided Zoho ops). Data honesty rules throughout: missing is "Missing"/"Unknown" (never a fake zero), received is labeled `Actual` vs `Supplier-declared`, negative remaining renders negative with an **Over-consumed** warning (never clamped), expected remaining (received − produced) and **recorded** remaining (terminal allocation ending balance / DEPLETED = 0, with source label: supervisor estimate, system-derived, allocation closeout, admin correction) are shown side by side with the difference when they disagree. Recovered/excluded runs contribute nothing to produced and are flagged. Multi-workflow (split/reused) bags show total produced + run count + "Multiple workflows used this bag". Fail-closed next action per bag (Start workflow / Finalize / Resolve remaining / Issue finished lot / Release lot / Queue Zoho / Fix Zoho mapping / Done / Needs review). 21 unit tests.
- **Receive Detail (`/inbound/[id]`):** new per-bag **Production** column rendering the breakdown (metrics line, source + output counts, workflow state, next action, warnings) via the shared `BagProductionSummaryInline` component. No box-number dependency introduced.
- **PO Closeout index (`/po-closeout`):** **Active / Closed / All tabs (Active default)** with per-PO chips (receives, bags, done, open, Zoho blockers) from a cheap SQL rollup (`listCloseoutPoIndexRollups`). Bucketing is a pure, conservative classifier (`classifyPoCloseoutIndexBucket`, unit-tested): CLOSED only when every received bag is resolved (excluded-without-recovery, or RELEASED/SHIPPED lot with Zoho queued/committed or Zoho disabled) with no open allocations and no Zoho blockers on a RECEIVED/CLOSED PO; CANCELLED counts as history; **all ambiguity → Active** so nothing unresolved can hide. Search covers PO number + vendor. Production dry-run: 86 Active / 1 Closed.
- **PO Closeout detail (`/po-closeout/[poId]`):** per-bag **Production** column (received/produced/remaining/%, output counts, source, partial/split/over-consumed/multi-run warnings) + read-only production-state filters (Has production / No production yet / Partial-split / Multiple runs / Over-consumed / Awaiting lot / Zoho blocked) that compose with the existing status tabs. Verdict logic, checklist, and batch actions untouched.
- **Source-bag context panels:** Production Output awaiting-lot rows ("Source bag" expandable panel), Partial Bag Workbench rows (expected-vs-recorded remaining in plain language, panel per bag), Finished Lot detail (side panel with received / produced / remaining after lot / allocation closeout / Zoho status), and Recall raw-bag results (renders for raw-bag-only, awaiting-lot, and recovered states — no finished lot required).
- **Page titles (PAGE-TITLES-1):** root layout now uses a Next.js title template (`Luma — %s`); 16 key admin pages export page-specific titles (PO Closeout, Production Output, Finished Lots, Traceability Lookup, Partial Bag Workbench, Workflows, Receiving, Zoho Production Output, Metrics, Settings, Bag Genealogy, QC Review, Live Floor, Dashboard, Receive Raw Pills, PO Reconciliation) and dynamic pages include identifiers ("Receive <name>", "PO Closeout <PO#>", "Finished Lot <number>") via cheap `generateMetadata` lookups. Structural tests pin all of it.

### Notes
- **Read-only feature: no mutation on page load anywhere** — no auto-issue/auto-release/Zoho queue/commit, no QR/allocation/workflow/lot changes, no new ledger. All numbers derive from existing canonical sources; the only behavior change is display/classification (index bucketing + titles). Dry-run on PO-00238/PO-00206 (78 bags): 50 with clear produced counts, 20 not started, 2 multi-workflow bags (352178, 352171), 8 over-consumed on PO-00238 (wrong-product signature), 10 expected-vs-recorded mismatches surfaced honestly, 12 NEEDS_MAPPING Zoho ops flagged. v1.18 auto-issue, v1.19 auto-release, v1.20/21 QR repair, v1.22/v1.22.1 closeout + Zoho done semantics, partial-bag/rebase, and v1.23.0 correction wizard all unchanged (regression suites green).

## [1.23.0] — 2026-07-07

### Added — ADMIN-CORRECTION-WIZARD-1: real corrections for wrong product / wrong route mistakes (recovery no longer quarantine-only)
- **Why:** admins expected "Wrong route / assignment recovery" to *fix* staff mistakes; it only quarantined. The payload's `intended_product_id` existed but was hardcoded `null` and there was no field to pick the correct product (observed on receipt 352182: Hyroxi MIT B - Choco Drift selected for a bag that physically ran FIX Beyond - Cocoa Calm — allocation showed an impossible 23,520 tablets consumed from a 7,223-pill bag).
- **Admin Correction Wizard** (`_workflow-recovery-form.tsx`, replaces the flat recovery form on Workflow Submissions): four explicit correction types with per-type copy (what changes / what is preserved / downstream effects) — **Wrong product selected**, **Wrong route / wrong production type**, **Wrong QR / receipt assigned**, **Quarantine only**. Every apply requires a detailed reason (min 10 chars) and an explicit confirmation checkbox.
- **Wrong-product correction (new):** required "Correct product" selector (only active, same-route products that allow the bag's tablet type are offered), current→correct product/route comparison, and a mandatory **downstream-impact preview** — old/new units from the submitted counts, old/new expected tablet consumption, bag-balance impact, finished lot impact, Zoho impact, PO Closeout impact — before an audited apply. Apply (in one transaction, `lib/production/wrong-product-correction-service.ts`): updates `workflow_bags.product_id`, appends a `PRODUCT_MAPPED` event (`source: ADMIN_WRONG_PRODUCT_CORRECTION`, with old/new product, reason, counts snapshot), reprojects `read_bag_metrics` (+ sku/station/material read models) under the corrected product, rewrites the single terminal allocation session's consumption (`RAW_BAG_ADJUSTED` ledger event + audit), rebuilds an existing finished lot under the corrected product and places it **ON_HOLD** for re-review (passport reprojected, `finished_lot_inputs` consumption updated), voids uncommitted Zoho production output ops (fresh preview/queue required), and writes `workflow_submissions.wrong_product_correction` audit with the full before/after snapshot. Station history is never touched.
- **Fail-closed compatibility evaluator** (`lib/production/wrong-product-correction.ts`, pure + fully unit-tested): blocks on route mismatch or VARIETY (`ROUTE_INCOMPATIBLE` — no card↔bottle conversion exists), committed Zoho output (`ZOHO_COMMITTED`), shipped/recalled lots, incomplete product setup (tablets-per-unit / packaging structure), tablet type not allowed, already-quarantined workflows, OPEN or ambiguous allocation sessions, and corrections whose consumption would exceed the known starting balance (`NEGATIVE_REMAINING`). Blocked cases show the exact reason + recommended action; incompatible cases are steered to the wrong-route flow.
- **Wrong-route correction (enhanced quarantine + restart):** records the **intended product** (`intended_product_id` now populated, plus additive `intended_route` and `correction_mode: QUARANTINE_AND_RESTART`) on the `WORKFLOW_RECOVERY` payload, explains that output is voided from normal sync while history is preserved, never offers route conversion, and links to Start Production after recording. Wrong-QR remains quarantine-only (relink deferred; copy says so). Legacy quarantine behavior unchanged (`correction_mode: QUARANTINE_ONLY`).
- **PO Closeout Command Center:** recovered rows no longer collapse into DONE — `WRONG_ROUTE_RECOVERED` → NEEDS_REVIEW "Wrong route recovered — start correct workflow" (action: start workflow), `VOIDED_FROM_OUTPUT` → NEEDS_REVIEW "manual review needed", `EXTERNAL_RECOVERY_REQUIRED` → BLOCKED "Zoho already committed — manual intervention required"; unknown recovery statuses fail closed to NEEDS_REVIEW. Exclusions without a recovery status stay DONE (legacy behavior preserved).
- **Visibility:** workflow detail shows a "Product corrected by admin" banner (original → corrected + reason) and a `PRODUCT_MAPPED` timeline badge; corrected bags set `read_bag_state.has_correction`. Production Output, Finished Lots, and Zoho ops pick up the corrected product automatically (they all derive product from `workflow_bags.product_id`).

### Notes
- **No DB migration** (no new enum values; new payload fields are additive JSONB). **No deploy-time mutation, no auto Zoho queue/commit, no auto-release** — corrected lots are held for explicit re-review; new Zoho ops require explicit preview/queue. No duplicate lots (existing lot is rebuilt in place) and no duplicate ops (voided ops free the per-lot unique slot). 5,000+ tests green including new pure-evaluator, classifier, and source-contract suites.
