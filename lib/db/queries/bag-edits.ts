  ...
  return db.transaction(async (tx) => {
    ...
    if (input.bagQrCode !== undefined && input.bagQrCode !== bag.bagQrCode) {
      ...
      if (newToken) {
        ...
        if (!newCard)
          return { ok: false as const, error: `QR card "${newToken}" not found.` };
        if (newCard.cardType === "VARIETY_PACK")
          return {
            ok: false as const,
            error: "Variety pack cards cannot be used for raw bags.",
          };
        ...
        if (newCard.status === "ASSIGNED" && newCard.assignedWorkflowBagId !== null)
          return {
            ok: false as const,
            error: "This QR card is already active in production.",
          };
