...
describe("queueAfterWorkAt", () => {
  it("routes a card bag being blistered toward the sealing queue", () => {
    expect(
      queueAfterWorkAt(graph, { ...CARD, stationKind: "BLISTER", priorEventTypes: [] }),
  ...
  it("treats handpack-blister and combined as card first ops", () => {
    for (const stationKind of ["HANDPACK_BLISTER", "COMBINED"]) {
      const dest = queueAfterWorkAt(graph, { ...CARD, stationKind, priorEventTypes: [] });
  ...
  it("routes sealing work toward packaging", () => {
    expect(
      queueAfterWorkAt(graph, { ...CARD, stationKind: "SEALING", priorEventTypes: [] })
        ?.queueStageKey,
  ...
  it("routes packaging work toward finished goods", () => {
    expect(
      queueAfterWorkAt(graph, { ...CARD, stationKind: "PACKAGING", priorEventTypes: [] }),
    ).toEqual({
  ...
  it("returns null for an unknown route or station kind", () => {
    expect(
      queueAfterWorkAt(graph, { routeCode: "NOT_A_ROUTE", stationKind: "SEALING", priorEventTypes: [] }),
    ).toBeNull();
    expect(
      queueAfterWorkAt(graph, { ...CARD, stationKind: "NOT_A_KIND", priorEventTypes: [] }),
    ).toBeNull();
  });

  it("never lists the working finishing station as eligible for its own output", () => {
    const dest = queueAfterWorkAt(graph, {
      routeCode: "BOTTLE",
      stationKind: "BOTTLE_CAP_SEAL",
  ...
  it("covers the sticker-only route end to end", () => {
    expect(
      queueAfterWorkAt(graph, { routeCode: "STICKER_ONLY", stationKind: "BOTTLE_STICKER", priorEventTypes: [] }),
    ).toEqual({ queueStageKey: "PACKAGING_QUEUE", eligibleStationKinds: ["PACKAGING"] });
    expect(
      queueAfterWorkAt(graph, { routeCode: "STICKER_ONLY", stationKind: "PACKAGING", priorEventTypes: [] }),
...
describe("deriveQueueTransition", () => {
  ...
  it("starts tracking when a bag is claimed at a first-op station", () => {
    for (const eventType of ["CARD_ASSIGNED", "BAG_CLAIMED"]) {
      const t = deriveQueueTransition(graph, { ...base, eventType });
      expect(t).toEqual({
        kind: "WORKING",
  ...
  it("marks the bag READY when the stage completes", () => {
    const t = deriveQueueTransition(graph, { ...base, eventType: "BLISTER_COMPLETE" });
  ...
  it("advances the destination when the next station picks the bag up", () => {
    const t = deriveQueueTransition(graph, {
      ...base,
      stationKind: "SEALING",
  ...
  it("unclaims on release and removes on finalize", () => {
    expect(deriveQueueTransition(graph, { ...base, eventType: "BAG_RELEASED" })).toEqual({
      kind: "UNCLAIM",
    });
    expect(deriveQueueTransition(graph, { ...base, eventType: "BAG_FINALIZED" })).toEqual({
      kind: "REMOVE",
    });
  });
  ...
  it("does nothing for non-flow events", () => {
    for (const eventType of ["BAG_PAUSED", "BAG_RESUMED", "OPERATOR_CHANGE", "MATERIAL_CONSUMED"]) {
      expect(deriveQueueTransition(graph, { ...base, eventType })).toEqual({ kind: "NONE" });
    }
  });

  it("does nothing without a station or route where one is required", () => {
    expect(
      deriveQueueTransition(graph, { ...base, stationId: null, eventType: "BAG_CLAIMED" }),
    ).toEqual({ kind: "NONE" });
    expect(
      deriveQueueTransition(graph, { ...base, routeCode: null, eventType: "BLISTER_COMPLETE" }),
    ).toEqual({ kind: "NONE" });
  });

  it("keeps a bottle bag out of packaging until both finishing steps ran", () => {
    const afterFirst = deriveQueueTransition(graph, {
      stationId: "st-9",
      stationKind: "BOTTLE_CAP_SEAL",
      routeCode: "BOTTLE",
...
describe("queueRank", () => {
  it("ranks queues downstream-increasing per route, finishing queues tied", () => {
    expect(queueRank(graph, "CARD_BLISTER", "SEALING_QUEUE")).toBe(1);
    expect(queueRank(graph, "CARD_BLISTER", "PACKAGING_QUEUE")).toBe(2);
    expect(queueRank(graph, "BOTTLE", "BOTTLE_STICKER_QUEUE")).toBe(
      queueRank(graph, "BOTTLE", "BOTTLE_INDUCTION_QUEUE"),
    );
    expect(queueRank(graph, "NOT_A_ROUTE", "SEALING_QUEUE")).toBeNull();
  });
});
